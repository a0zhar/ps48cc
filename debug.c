...
static char* do_ty2s(Dict* dict, Type* ty) {
	...
	switch (ty->kind) {
		...
		case KIND_FUNC: {
				...
				buf_printf(b, "(");
				if (ty->params) {
					int params_length = vec_len(ty->params);
...
static void do_node2s(Buffer* b, Node* node) {
	...
	switch (node->kind) {
		case AST_LITERAL:
			switch (node->ty->kind) {
				case KIND_CHAR: {
						long ivalType = node->ival;
						if (ivalType == '\n')     buf_printf(b, "'\n'");
						else if (ivalType == '\\')buf_printf(b, "'\\\\'");
						else if (ivalType == '\0')buf_printf(b, "'\\0'");
			...
			break;
		case AST_LABEL:buf_printf(b, "%s:", node->label);break;
		case AST_LVAR:
			...
			break;
		case AST_GVAR:buf_printf(b, "gv=%s", node->varname);break;
		case AST_FUNCALL:
		case AST_FUNCPTR_CALL:
			buf_printf(b, "(%s)%s(", ty2s(node->ty), node->kind == AST_FUNCALL ? node->fname : node2s(node));
			for (int i = 0, maxlen = vec_len(node->args); i < maxlen; i++) {
				if (i > 0)
					buf_printf(b, ",");
			...
			buf_printf(b, ")");
			break;
		case AST_FUNCDESG: buf_printf(b, "(funcdesg %s)", node->fname);break;
		case AST_FUNC:
			buf_printf(b, "(%s)%s(", ty2s(node->ty), node->fname);
			for (int i = 0, maxlen = vec_len(node->params); i < maxlen; i++) {
				if (i > 0) buf_printf(b, ",");
			...
			break;
		case AST_GOTO:buf_printf(b, "goto(%s)", node->label);break;
		case AST_DECL:
			buf_printf(b, "(decl %s %s", ty2s(node->declvar->ty), node->declvar->varname);
			if (node->declinit) {
				buf_printf(b, " ");
		...
		case AST_CONV:
			...
			break;
		case AST_IF:
			buf_printf(b, "(if %s %s", node2s(node->cond), node2s(node->then));
			if (node->els) buf_printf(b, " %s", node2s(node->els));
			buf_printf(b, ")");
			break;
		case AST_TERNARY:
			buf_printf(b, "(? %s %s %s)", node2s(node->cond), node2s(node->then), node2s(node->els));
			break;
		case AST_RETURN:buf_printf(b, "(return %s)", node2s(node->retval)); break;
		case AST_COMPOUND_STMT:
			buf_printf(b, "{");
			for (int i = 0, maxlen = vec_len(node->stmts); i < maxlen; i++) {
		...
		case '|': binop_to_string(b, "|", node); break;
		case OP_CAST:
			buf_printf(b, "((%s)=>(%s) %s)", ty2s(node->operand->ty), ty2s(node->ty), node2s(node->operand));
			break;
		case OP_LABEL_ADDR: buf_printf(b, "&&%s", node->label); break;
		default:
			if (node->kind == OP_EQ) buf_printf(b, "(== ");
			else buf_printf(b, "(%c ", node->kind);
			buf_printf(b, "%s %s)", node2s(node->left), node2s(node->right));
			break;
	}
}
...
static char* encoding_prefix(int enc) {
	switch (enc) {
		case ENC_CHAR16: return "u";  // if the encoding is 16-bit char
		case ENC_CHAR32: return "U";  // if the encoding is 32-bit char
		case ENC_UTF8:   return "u8"; // if the encoding is UTF-8 (8 bit Unicode Transformation Format)
		case ENC_WCHAR:  return "L";  // if the encoding is WCHAR (wide-characters)
		default: break;               // if encoding is none of above (exit switch and return "")
	}
	return "";
}
